...
export class AstTryStatement extends AstNode {
	constructor(parseTree: any) {
		...
		this.handler = parse(parseTree.handler);
	}
...
export class AstThrowStatement extends AstNode {
	...
	public compile(compiler: Compiler): void {
		...
		compiler.emit("Throw statement", this, (runtime: Runtime) => {
			...
			// We'll just stuff this into the return value slot. It more or less works out
			// the same way, anyway - return and throw are pretty analogous. Throw just works
			// across multiple call stack frames.
